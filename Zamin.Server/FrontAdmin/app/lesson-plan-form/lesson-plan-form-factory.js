(function() {
  ...
  function LessonPlanForm($http, consts) {
    ...
    LessonPlanFormBase.save = function(lessonPlan) {
      ...
      if (lessonPlan.PresentationFiles != undefined) {
        lessonPlan.PresentationFiles.forEach(function(file, index) {
          fd.append("PresentationFiles[" + index + "]", file);
        });
      }
      if (lessonPlan.WrittenLessonPlansFiles != undefined) {
        lessonPlan.WrittenLessonPlansFiles.forEach(function(file, index) {
          fd.append("WrittenLessonPlansFiles[" + index + "]", file);
        });
      }
