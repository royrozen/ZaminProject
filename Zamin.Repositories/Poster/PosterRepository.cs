...
namespace Zamin.Repositories
{
  public class PosterRepository:RepositoryBase<DataContext> , IPosterRepository
  {
      ...
      public bool UpdatePoster(PosterWebModel poster)
      {
          ...
          var imageFileName = dbModel.ImageFileName;
          ...
          dbModel.ImageFileName = imageFileName;
        ...
        public bool LikePoster(string loginToken, int posterId)
        {
            var user = DataContext.WebsiteUsers.FirstOrDefault(u => u.LoginToken == loginToken);
            var poster = DataContext.Poster.Include(a => a.WebsiteUserPosterLike).FirstOrDefault(l => l.Id == posterId);
            if (user == null || poster == null) return false;
            if (!poster.WebsiteUserPosterLike.Any(l => l.Id == user.Id))
            {
                poster.WebsiteUserPosterLike.Add(user);
                poster.NumOfLikes++;
            }
            return Save();
        }
