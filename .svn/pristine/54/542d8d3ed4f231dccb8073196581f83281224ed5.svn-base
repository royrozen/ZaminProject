(function() {
  ...
  function RegistrationCtrl($scope, $rootScope,$timeout, $location, Registration) {
    $scope.signupInfo = {
      IsMale: "true",
      UserTypeId: "1",
    ...
    $scope.tags=$('#tagSelect').val();
    $scope.tags.forEach(function(tag, index) {
              // $scope.signupInfo.Tags[index] = tag;
              $scope.signupInfo.Tags.push({Id: index, TagName :tag})
              // $scope.signupInfo.Tags.push("Tags[" + index + "].TagName", tag.TagName);
            });
